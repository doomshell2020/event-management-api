const EventTicketType = require('../../../models/ticket.model');
const Event = require('../../../models/event.model'); // Assuming event model exists
...
module.exports.createTicket = async (req) => {
    try {
        ...
        const existingTicket = await EventTicketType.findOne({
            where: {
                eventid: event_id,
                title: title.trim()
            }
        ...
        // ✅ Create ticket
        const newTicket = await EventTicketType.create({
            eventid: event_id,
            userid: user_id,
            title: title.trim(),
...
module.exports.updateTicket = async (req) => {
    try {
        ...
        const existingTicket = await EventTicketType.findByPk(ticketId);
        if (!existingTicket) {
            return {
                success: false,
        ...
        if (existingTicket.eventid && title) {
            const duplicateTicket = await EventTicketType.findOne({
                where: {
